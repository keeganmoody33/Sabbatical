...
| `adjudication/FREEZE-2.md` | Freeze 2 accounting: 40 Jobright rows, 98 LinkedIn rows, 56 overlaps, 77 net-new, full census 298 | 4 | 2 | 4 | 3 | 2 | 3 | "Interviewed in full census: 13 (platform files carry no interview events)" The full census grows by 77 rows and the numerator stays fixed, so the reported rate falls from 13/221 to 13/298 for a purely structural reason. The parenthetical says so. Nothing forces a reader to notice that this is not a finding about the search. |
...
Any reader comparing 13/221 against 13/298 is looking at a measurement artifact, not a change in outcomes. `RESULTS.md` reports both as unreduced fractions, which is the right instinct, and neither the results draft nor the freeze note states plainly that the second fraction cannot differ from the first.
...
| No `ambiguous` match status, so a refused match was emitted as `net_new`. **Fixed on this branch.** | `ingest_platform.py` correctly refused to match when tier 3 yielded more than one candidate, then emitted the record as `net_new`, indistinguishable from a genuinely new one. `match_status` carried three values across all 134 rows of `adjudication/platform_match.csv`: `net_new` 77, `overlap` 56, `opportunity_or_non_census` 1. The refusal was therefore invisible downstream and the 298-row census carried no "of which K unresolved" caveat. Surfaced by a review bot on this branch, verified, and fixed: `match_freeze1` now returns a third list, ambiguous rows carry `match_status = ambiguous` plus a `candidate_parent_ids` column, and they are held out of the census. Zero rows hit the branch on the current corpus, so every published figure is unchanged. A second review round found the same defect one tier earlier: the exact-key and unspecified-role lookups can also return several candidates, because `role_key` omits cycle while the identity key includes it. One row hits it, a FOSSA platform row matching both `fossa|unspecified|c1` and `fossa|unspecified|c2`, where the parent was taken in file order. Those are overlaps either way so the census never moved. A third round established that recording the candidates was not sufficient on its own, because a populated `parent_id` beside `match_status = overlap` still asserts a resolution: those rows now carry `overlap_parent_ambiguous` with an empty `parent_id`. Overlap totals are unchanged at 56, reported as 55 resolved plus 1 unresolved. |
