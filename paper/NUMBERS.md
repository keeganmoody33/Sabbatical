...
| Application to interview rate | 10/223 | `views/funnel_by_role_lane.csv` | sum `n_interviewed` over sum `n_applications` |
...
| Zero-interview lanes, and applications in them | 5 lanes, 83 applications | same, rows where `n_interviewed = 0` |
| Upper bound range on zero lanes | 0.110 to 0.324 | same, `interview_rate_wilson_hi` |
| Distinct titles in explicit GTM engineering | 37 | `views/title_language.csv`, `n_distinct_titles` |
...
| interview set found by both coders | 9 of 10, `adjudication/LATENCY.md` |
...
| 37 titles in one lane | 37 | `views/title_language.csv` |
| Applications in lanes producing zero interviews | 83 of 223 | `views/funnel_by_role_lane.csv` |
...
| Census rows coded `unspecified` before Freeze 3.1 | 40 | `adjudication/title_backfill.csv` |
| Titles backfilled from this repository's own platform artifacts | 11 | same |
| Unspecified role lanes, before and after | 36 to 25 | `views/funnel_by_role_lane.csv` |
| Titles the challenger resolves from outside evidence, not adopted | 19 | `challenge/CHALLENGE.md` section 3.1 |
...
- 221 applications and 13/221. Superseded at Freeze 3 by 223 and 14/223, then at Freeze 4 by 11/223 and 10/223.
